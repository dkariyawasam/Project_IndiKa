...
static void InitTrainerTowerFloor(void)
{
    if (gMapHeader.mapLayoutId - LAYOUT_ROCKET_LEAGUE_LOBBY > sTrainerTowerState->data.numFloors)
    {
...
static void GetFloorAlreadyCleared(void)
{
    ...
    if (mapLayoutId - LAYOUT_TRAINER_TOWER_1F == TRAINER_TOWER.floorsCleared
     && mapLayoutId - LAYOUT_ROCKET_LEAGUE_LOBBY <= CURR_FLOOR.floorIdx)
...
static void ShouldWarpToCounter(void)
{
    if (gMapHeader.mapLayoutId == LAYOUT_ROCKET_LEAGUE_LOBBY && VarGet(VAR_MAP_SCENE_TRAINER_TOWER) == 0)
        gSpecialVar_Result = FALSE;
    else
        gSpecialVar_Result = FALSE;
}
