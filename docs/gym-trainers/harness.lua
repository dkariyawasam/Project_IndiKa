...
function qa.capture(name)
 local base='/Users/deekariyawasam/pokefirered/docs/gym-trainers/'..name
 emu:screenshot(base..'.png');emu:saveStateFile('/tmp/gym-trainers/state-'..name..'.ss')
...
qa.poll=callbacks:add('frame',function()
 if qa.frame%30==0 then
  local ok,err=pcall(dofile,'/tmp/gym-trainers/commands.lua')
  if not ok then console:error(err) end
  local g,m,x,y=qa.state();local f=io.open('/tmp/gym-trainers/status.json','w')
