// HGSS canvas conversion using nearest-neighbour scaling; no generated artwork.
...
const root='docs/mamoswine-sprites/source/',out='graphics/pokemon/mamoswine/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:0,top:7,width:80,height:65}:{left:1,top:30,width:79,height:42};
 const w=64,h=side==='front'?52:34;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?10:30;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/mamoswine-'+side+'.4bpp',Buffer.from(tiles));
}
})();
