...
const root='docs/spiritomb-sprites/source/',out='graphics/pokemon/spiritomb/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:10,top:10,width:57,height:56}:{left:0,top:6,width:79,height:68};
 const w=side==='front'?57:64,h=side==='front'?56:55;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?4:9;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/spiritomb-'+side+'.4bpp',Buffer.from(tiles));
}
})();
