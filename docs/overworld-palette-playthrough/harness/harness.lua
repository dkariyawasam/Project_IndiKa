...
function qa.capture(name)
 local base='/Users/deekariyawasam/pokefirered/docs/overworld-palette-playthrough/'..name
 emu:screenshot(base..'.png');emu:saveStateFile('/tmp/overworld-palette-playthrough/state-'..name..'.ss')
...
qa.poll=callbacks:add('frame',function()
 if qa.frame%30==0 then
  local ok,err=pcall(dofile,'/tmp/overworld-palette-playthrough/commands.lua')
  if not ok then console:error(err) end
  local g,m,x,y=qa.state();local f=io.open('/tmp/overworld-palette-playthrough/status.json','w')
