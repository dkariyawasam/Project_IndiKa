...
const root='docs/eelektross-sprites/source/',out='graphics/pokemon/eelektross/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:10,top:20,width:77,height:59}:{left:7,top:17,width:83,height:62};
 const w=64,h=side==='front'?49:48;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?13:16;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/eelektross-'+side+'.4bpp',Buffer.from(tiles));
}
})();
