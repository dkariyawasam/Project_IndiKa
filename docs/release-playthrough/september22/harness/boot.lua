qa={frame=0,queue={},step=1,timer=0,lastText="", sym={gMain=50344544,gSaveBlock1Ptr=50352504,gPlayerParty=33702728,gPlayerPartyCount=33702125,gStringVar4=33693156,sLockFieldControls=50335644,CB2_Overworld=134572220,gBattleTypeFlags=33696784,gBattleOutcome=33701710}}
qa.chars={}
qa.chars[0]=" "
qa.chars[1]="À"
qa.chars[2]="Á"
...
function qa.snapshot()
 local g,m,x,y=qa.state();local f=io.open('/tmp/kanto-release-september22/status.txt','w')
 ...
 f:write(qa.text());f:close();emu:screenshot('/tmp/kanto-release-september22/current.png')
end
...
qa.cb=callbacks:add('frame',function()
 ...
 if qa.frame%60==0 then
  local t=qa.text();if t~=qa.lastText then local f=io.open('/tmp/kanto-release-september22/dialogue.txt','a');f:write(t..'\n');f:close();qa.lastText=t end
  local f=io.open('/tmp/kanto-release-september22/command.lua','r')
  if f then local s=f:read('*a');f:close();if #s>0 then f=io.open('/tmp/kanto-release-september22/command.lua','w');f:close();local fn,err=load(s);if fn then local ok,e=pcall(fn);if not ok then console:error(e) end else console:error(err) end end end
  qa.snapshot()
 end
end)
qa.snapshot()
