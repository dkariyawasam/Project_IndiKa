...
qa.guard=callbacks:add('frame',function()
 ...
 local action=false
 for _,i in ipairs(emu:read32(qa.sym.gBattleTypeFlags)%2==1 and {0,2} or {0}) do if emu:read32(qa.bsym.controllers+i*4)==qa.bsym.action+1 then action=true end end
 if not action then return end
 for _,i in ipairs(emu:read32(qa.sym.gBattleTypeFlags)%2==1 and {0,2} or {0}) do
