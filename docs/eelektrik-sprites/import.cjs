...
const root='docs/eelektrik-sprites/source/',out='graphics/pokemon/eelektrik/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:23,top:16,width:49,height:63}:{left:23,top:15,width:50,height:67};
 const w=side==='front'?49:48,h=side==='front'?63:64;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?1:0;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/eelektrik-'+side+'.4bpp',Buffer.from(tiles));
}
})();
