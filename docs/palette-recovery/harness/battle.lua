...
qa.testRunning=true;dofile('/tmp/palette-recovery/pilot.lua')
...
qa.battleCheck=callbacks:add('frame',function()
 ...
 if emu:read32(qa.sym.gMain+4)==qa.sym.BattleMainCB2+1 then
  if not qa.battleSeen then emu:screenshot('/Users/deekariyawasam/pokefirered/docs/palette-recovery/37-lab-battle.png')end
  qa.battleSeen=true
 end
 if qa.battleSeen and emu:read8(qa.sym.gBattleOutcome)~=0 then
  qa.testRunning=false
  local f=io.open('/tmp/palette-recovery/battle-result.txt','w');f:write(tostring(emu:read8(qa.sym.gBattleOutcome)));f:close()
  qa.run({{kind='text',frames=900},{kind='wait',frames=120},{kind='capture',name='14-lab-after-battle'}})
 end
end)
