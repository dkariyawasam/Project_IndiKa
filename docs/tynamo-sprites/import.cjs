...
const root='docs/tynamo-sprites/source/',out='graphics/pokemon/tynamo/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:29,top:40,width:38,height:17}:{left:29,top:39,width:38,height:18};
 const w=38,h=side==='front'?17:18;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?24:46;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/tynamo-'+side+'.4bpp',Buffer.from(tiles));
}
})();
