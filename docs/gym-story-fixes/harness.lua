...
function qa.capture(name)
 local base='/Users/deekariyawasam/pokefirered/docs/gym-story-fixes/'..name
 emu:screenshot(base..'.png');emu:saveStateFile('/tmp/gym-fixes/state-'..name..'.ss')
...
qa.poll=callbacks:add('frame',function()
 if qa.frame%30==0 then
  local ok,err=pcall(dofile,'/tmp/gym-fixes/commands.lua')
  if not ok then console:error(err) end
  local g,m,x,y=qa.state();local f=io.open('/tmp/gym-fixes/status.json','w')
