...
const root='docs/dusknoir-sprites/source/',out='graphics/pokemon/dusknoir/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:2,top:4,width:73,height:65}:{left:0,top:13,width:80,height:59};
 const w=64,h=side==='front'?57:47;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?5:17;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/dusknoir-'+side+'.4bpp',Buffer.from(tiles));
}
})();
