...
const root='docs/leafeon-sprites/source/',out='graphics/pokemon/leafeon/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:14,top:15,width:50,height:50}:{left:11,top:3,width:54,height:77};
 const w=side==='front'?50:45,h=side==='front'?50:64;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?10:0;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/leafeon-'+side+'.4bpp',Buffer.from(tiles));
}
})();
