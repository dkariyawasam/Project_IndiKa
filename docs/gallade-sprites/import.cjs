...
const root='docs/gallade-sprites/source/',out='graphics/pokemon/gallade/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:14,top:5,width:56,height:70}:{left:3,top:5,width:73,height:71};
 const w=side==='front'?51:64,h=side==='front'?64:62;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?0:2;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/gallade-'+side+'.4bpp',Buffer.from(tiles));
}
})();
