...
const root='docs/porygon_z-sprites/source/',out='graphics/pokemon/porygon_z/';
(async()=>{
let pal=[[0,0,0]],shiny=[[0,0,0]],frames={};
for(const side of ['front','back']){
 const bounds=side==='front'?{left:22,top:15,width:42,height:51}:{left:0,top:10,width:75,height:62};
 const w=side==='front'?42:64,h=side==='front'?51:53;
 ...
 const left=Math.floor((64-w)/2),top=side==='front'?8:11;
...
for(const [side,pixels] of Object.entries(frames)){
 ...
 fs.writeFileSync('/tmp/porygon_z-'+side+'.4bpp',Buffer.from(tiles));
}
})();
