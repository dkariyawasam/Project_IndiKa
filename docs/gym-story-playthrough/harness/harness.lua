...
function qa.capture(name)
 local base='/Users/deekariyawasam/pokefirered/docs/gym-story-playthrough/'..name
 emu:screenshot(base..'.png');emu:saveStateFile('/tmp/gym-story-qa/state-'..name..'.ss')
...
qa.callback=callbacks:add('frame',function()
 ...
 if c.untilField and emu:read32(0x03003264)==0x08056591 then done=true end
 if done then
...
qa.poll=callbacks:add('frame',function()
 if qa.frame%30==0 then
  local ok,err=pcall(dofile,'/tmp/gym-story-qa/commands.lua')
  if not ok then console:error(err) end
  local g,m,x,y=qa.state();local f=io.open('/tmp/gym-story-qa/status.json','w')
