...
for base,species,source_path,footbase in [('farfetchd','FARFETCHD_GALARIAN','farfetchd/galar','FARFETCHD'),('meowth','MEOWTH_GALARIAN','meowth/galar','MEOWTH'),('perrserker','PERRSERKER','perrserker','MEOWTH'),('corsola','CORSOLA_GALARIAN','corsola/galar','CORSOLA'),('cursola','CURSOLA','cursola','CORSOLA'),('weezing','WEEZING_GALARIAN','weezing/galar','WEEZING')]:
 isrose=False;name=species.lower();symbol='Galarian'+base.capitalize();src=S/source_path;out=R/'graphics/pokemon'/name;out.mkdir(exist_ok=True);archive=D/'source'/name;archive.mkdir(parents=True,exist_ok=True)
 for f in ['front.png','back.png','icon.png','normal.pal','shiny.pal']:shutil.copy2(src/f,archive/f)
 for side in ['front','back']:
  im=Image.open(src/('front.png' if side=='front' else 'back.png')).crop((0,0,64,64));im.info['transparency']=0;im.save(out/(side+'.png'),bits=4,transparency=0)
 ...
 if (src/'footprint.png').exists():
  shutil.copy2(src/'footprint.png',out/'footprint.png')
  sym='gMonFootprint_'+symbol
  defs+=f'\nconst u8 {sym}[] = INCBIN_U8("graphics/pokemon/{name}/footprint.1bpp");'
  decls.append(f'extern const u8 {sym}[];');map_symbol('footprint',species,sym)
 else:
  standard=re.search(r'\[SPECIES_'+footbase+r'\]\s*=\s*(\w+)',files['footprint'])[1];map_symbol('footprint',species,standard)
...
print('Imported all six remaining Galarian forms/evolutions.')
