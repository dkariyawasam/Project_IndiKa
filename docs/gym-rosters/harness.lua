...
function qa.capture(name)
 local base='/Users/deekariyawasam/pokefirered/docs/gym-rosters/'..name
 emu:screenshot(base..'.png');emu:saveStateFile('/tmp/gym-rosters/state-'..name..'.ss')
...
qa.poll=callbacks:add('frame',function()
 if qa.frame%30==0 then
  local ok,err=pcall(dofile,'/tmp/gym-rosters/commands.lua')
  if not ok then console:error(err) end
  local g,m,x,y=qa.state();local f=io.open('/tmp/gym-rosters/status.json','w')
